...
export function ProjectCard({
	...
	if (isFeatured) {
		return (
			<article
				className={`group relative flex flex-col justify-between overflow-hidden rounded-3xl bg-accent p-7 text-accent-foreground transition-all duration-500 hover:-translate-y-1 ${colSpan}`}
				data-reveal
			>
				<div>
	...
	return (
		<article
			className={`group relative flex flex-col justify-between overflow-hidden rounded-3xl border border-border bg-card p-7 transition-all duration-500 hover:-translate-y-1 hover:border-accent/40 hover:shadow-[0_30px_60px_-30px_rgba(124,58,237,0.35)] ${colSpan}`}
			data-reveal
		>
			<div>
				<div className="flex items-start justify-between gap-3">
					...
					{projeto.github && (
						<a
							...
							className="grid h-9 w-9 shrink-0 place-items-center rounded-full border border-border text-muted-foreground transition-colors hover:border-accent/40 hover:text-accent"
						>
